...
| 백엔드 | **장면 사용자 메모/태그 API** | ✅ 완료 |
| 백엔드 | **장면 태그 삭제 API** | ✅ 완료 |
| 백엔드 | **사진 태그 삭제 API** | ✅ 완료 |
| 프론트엔드 | **태그 삭제 UI (장면, 사진)** | ✅ 완료 |
| 프론트엔드 | **검색→장면 네비게이션** | ✅ 완료 |
| 프론트엔드 | **검색 결과 URL 캐싱** | ✅ 완료 |
...
| 장면 user_notes 저장 | ✅ 성공 | 2026-01-27 |
| 장면 태그 삭제 | ✅ 성공 | 2026-01-27 |
| 사진 태그 삭제 | ✅ 성공 | 2026-01-27 |
| 검색→장면 네비게이션 | ✅ 성공 | 2026-01-27 |
| 검색 결과 URL 캐싱 | ✅ 성공 | 2026-01-27 |
| 장면 타임라인 정확한 표시 | ✅ 성공 | 2026-01-27 |
...
Phase 8: 태그 관리 기능 ✅ (신규)
    ├── Step 8.1: 장면 사용자 메모/태그 추가 ✅
    ├── Step 8.2: 태그 삭제 기능 (장면, 사진) ✅
    └── Step 8.3: 태그 UI 개선 (삭제 버튼) ✅

Phase 9: 검색 네비게이션 ✅ (신규)
    ├── Step 9.1: 검색 결과에서 장면 선택 시 동영상 이동 ✅
    ├── Step 9.2: URL 파라미터 기반 검색 상태 유지 ✅
    └── Step 9.3: 뒤로가기 시 검색 결과 자동 복원 ✅

Phase 10: 외부 API 연동 (미구현)
    ├── Step 10.1: Google Nanobana API 연동
    └── Step 10.2: 동영상 제작 요청 기능
```
...
| 파일 | 수정 내용 |
|------|-----------|
| `backend/app/models/scene.py` | user_notes 필드 추가 |
| `backend/app/api/routes/scenes.py` | PUT /{id} 엔드포인트 추가 |
| `frontend/src/app/videos/[id]/page.tsx` | 장면 메모 편집 UI |

```python
# scenes.py
@router.put("/{scene_id}")
async def update_scene(scene_id: UUID, data: dict, db: Session = Depends(get_db)):
    scene = db.query(Scene).filter(Scene.id == scene_id).first()
    if "user_notes" in data:
        scene.user_notes = data["user_notes"]
        # #태그 파싱 및 저장
        parse_and_save_user_tags(scene, data["user_notes"], db)
    db.commit()
    return scene
```
...
| 파일 | 수정 내용 |
|------|-----------|
| `backend/app/api/routes/scenes.py` | DELETE /{id}/tags/{tag_id} |
| `backend/app/api/routes/images.py` | DELETE /{id}/tags/{tag_id} |
| `frontend/src/lib/api.ts` | deleteSceneTag, deleteImageTag 함수 |
...
| 파일 | 수정 내용 |
|------|-----------|
| `frontend/src/app/videos/[id]/page.tsx` | 장면 태그에 삭제 버튼 추가 |
| `frontend/src/app/images/[id]/page.tsx` | 사진 태그에 삭제 버튼 추가 |

### 테스트 결과

| 테스트 케이스 | 결과 | 비고 |
|---------------|------|------|
| 장면 user_notes 저장 | ✅ 성공 | #태그 자동 파싱 |
| 장면 태그 삭제 | ✅ 성공 | DB 즉시 반영 |
| 사진 태그 삭제 | ✅ 성공 | DB 즉시 반영 |
| 태그 삭제 UI | ✅ 성공 | hover 시 X 버튼 표시 |

---

## 10-2. Phase 9: 검색 네비게이션 ✅ (신규)

### 목표
- 검색 결과에서 장면 클릭 시 해당 동영상으로 이동하여 장면 자동 선택
- 뒤로가기 시 검색 결과 자동 복원
...
| 파일 | 수정 내용 |
|------|-----------|
| `frontend/src/app/search/page.tsx` | 장면 링크에 ?scene= 파라미터 추가 |
| `frontend/src/app/videos/[id]/page.tsx` | useSearchParams로 scene 파라미터 읽기, 장면 자동 선택 |

```typescript
// search/page.tsx
<Link href={`/videos/${scene.video_id}?scene=${scene.id}`}>
  {scene.title}
</Link>
...
useEffect(() => {
  if (sceneIdFromUrl && scenes?.scenes) {
    const targetScene = scenes.scenes.find(s => s.id === sceneIdFromUrl);
    if (targetScene) {
...
| 파일 | 수정 내용 |
|------|-----------|
| `frontend/src/app/search/page.tsx` | 검색 시 URL 파라미터 업데이트 |

```typescript
// 검색 시 URL 업데이트
const updateUrlParams = () => {
  const params = new URLSearchParams();
  if (andTags.length > 0) params.set('and', andTags.join(','));
  if (orTags.length > 0) params.set('or', orTags.join(','));
  if (notTags.length > 0) params.set('not', notTags.join(','));
...
| 파일 | 수정 내용 |
|------|-----------|
| `frontend/src/app/search/page.tsx` | useMutation → useQuery 변경, URL 파라미터 기반 자동 캐싱 |

```typescript
// useQuery로 변경하여 자동 캐싱
const { data: searchResults } = useQuery({
  queryKey: ['search', andParam, orParam, notParam],
  queryFn: async () => {
    const query: SearchQuery = { target: ['videos', 'scenes', 'images'] };
...
| 파일 | 수정 내용 |
|------|-----------|
| `frontend/src/components/SceneTimeline.tsx` | selectedSceneId prop 추가, 시간 기반 대신 선택 기반 표시 |
...
interface SceneTimelineProps {
  ...
  duration: number;
  currentTime: number;
  selectedSceneId?: string | null; // 추가
  onSceneClick: (scene: Scene) => void;
}

const currentScene = useMemo(() => {
  // selectedSceneId가 있으면 우선 사용
  if (selectedSceneId) {
...
| 테스트 케이스 | 결과 | 비고 |
|---------------|------|------|
| 검색 결과 장면 클릭 → 동영상 이동 | ✅ 성공 | ?scene= 파라미터로 전달 |
| 동영상 상세에서 장면 자동 선택 | ✅ 성공 | seekTo() 호출 |
| 뒤로가기 시 검색 결과 복원 | ✅ 성공 | useQuery 캐싱 |
| 장면 타임라인 정확한 표시 | ✅ 성공 | selectedSceneId 우선 사용 |
...
- 장면별 사용자 메모/태그 편집 가능
- URL 파라미터 ?scene={id}로 특정 장면 자동 선택
- 뒤로가기 버튼으로 이전 페이지(검색 등)로 복귀
...
- 원본 이미지 표시
- AI 설명 (편집 가능)
- 태그 목록 (AI 태그/사용자 태그 구분, 개별 삭제 가능)
- 사용자 정의 태그 추가 (#태그 형식)
- 태깅 시작 버튼
- 다운로드 버튼
- 뒤로가기 버튼으로 이전 페이지로 복귀

### 12.7 검색 페이지 (/search)
- 검색 모드: AND (파란색), OR (초록색), NOT (빨간색)
- 태그 입력 또는 버튼 클릭
- 검색 결과: 동영상, 사진, 장면 탭 구분
- URL 파라미터로 검색 상태 유지 (?and=태그1,태그2&or=태그3&not=태그4)
- 장면 클릭 시 해당 동영상 상세 페이지로 이동 + 장면 자동 선택
- 뒤로가기 시 검색 결과 자동 복원 (React Query 캐싱)
...
- 동영상, 사진, 장면 모두 user_notes 필드 지원
- 정규식 파싱: `r'#([\w가-힣]+)'`
- confidence = 1.0으로 마킹
- UI 표시: 보라색 배지

### 13.3 태그 삭제
- 동영상, 사진, 장면 모두 개별 태그 삭제 지원
- DELETE API: `/api/{type}/{id}/tags/{tag_id}`
- UI: hover 시 X 버튼 표시, 클릭 시 확인 후 삭제
- DB 즉시 반영 (video_tags, image_tags, scene_tags 테이블)
...
| v20260127-2 | 2026-01-27 | - | 태그 관리 기능 추가 (Phase 8), 검색 네비게이션 추가 (Phase 9), 장면 사용자 메모/태그, 태그 삭제, 검색 결과 URL 캐싱, 장면 타임라인 selectedSceneId |
