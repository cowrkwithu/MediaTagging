...
| user_notes | TEXT | | 사용자 입력 메모 (#태그 포함) |
| created_at | TIMESTAMP | NOT NULL | 생성일시 |

**정렬**: 장면은 항상 `start_time` 기준으로 정렬되어 조회됨
...
| DELETE | /{id}/tags/{tag_id} | 사진 태그 삭제 |
...
| DELETE | /{id}/tags/{tag_id} | 장면 태그 삭제 |
...
| v20260127-2 | 2026-01-27 | - | scenes 테이블 user_notes 필드 추가, scene_tags/image_tags confidence 필드 추가, 태그 삭제 API 추가, 장면 수정 API 추가 |
